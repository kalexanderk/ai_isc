...
if __name__ == "__main__":
  ...
  for maskfile in tqdm(maskfiles):
    masks = np.load(os.path.join(parsed_args.maskpath, maskfile), allow_pickle=True)
    ...
    for idx,fname in enumerate(fnames):
      with h5.File(os.path.join(parsed_args.datapath, fname.decode('unicode_escape')), "r") as f:
        data = f["climate"]["data"][...]
        labels = f["climate"]["labels"][...]

      print(masks["label"].shape)
      print(labels.shape)
      ...
      #do the plotting
      basename = os.path.join(parsed_args.outpath, os.path.splitext(fname.decode('unicode_escape'))[0])
      TMQ = data[0]
